 ...
 *  - Add support for History-Info header.
 *
						 ...
						 const pjsip_generic_array_hdr *rhs)
{
    ...
    pj_strdup(pool, &hdr->name, &rhs->name);
    pj_strdup(pool, &hdr->sname, &rhs->sname);
 ...
 * History-Info header.
 */
static int pjsip_history_info_hdr_print( pjsip_history_info_hdr *hdr,
				         char *buf, pj_size_t size);
static pjsip_history_info_hdr* pjsip_history_info_hdr_clone( pj_pool_t *pool,
					                     const pjsip_history_info_hdr *hdr);
static pjsip_history_info_hdr* pjsip_history_info_hdr_shallow_clone( pj_pool_t *pool,
				                                     const pjsip_history_info_hdr *hdr);

static pjsip_hdr_vptr history_info_hdr_vptr =
{
    (pjsip_hdr_clone_fptr) &pjsip_history_info_hdr_clone,
    (pjsip_hdr_clone_fptr) &pjsip_history_info_hdr_shallow_clone,
    (pjsip_hdr_print_fptr) &pjsip_history_info_hdr_print,
};

PJ_DEF(pjsip_history_info_hdr*) pjsip_history_info_hdr_init( pj_pool_t *pool,
						             void *mem )
{
    static const pj_str_t history_info_hdr_name = {"History-Info", 12};

    pjsip_history_info_hdr *hdr = (pjsip_history_info_hdr*) mem;

    PJ_UNUSED_ARG(pool);

    pj_bzero(mem, sizeof(pjsip_history_info_hdr));
    init_hdr(hdr, PJSIP_H_OTHER, &history_info_hdr_vptr);
    ((pjsip_hdr*)hdr)->name = history_info_hdr_name;
    ((pjsip_hdr*)hdr)->sname = history_info_hdr_name;
    pj_list_init(&hdr->other_param);
    return hdr;

}

PJ_DEF(pjsip_history_info_hdr*) pjsip_history_info_hdr_create(pj_pool_t *pool)
{
    void *mem = pj_pool_alloc(pool, sizeof(pjsip_history_info_hdr));
    return pjsip_history_info_hdr_init(pool, mem);
}
					 ...
					 char *buf, pj_size_t size)
{
    ...
    *buf++ = ' ';
    ...
    copy_advance_pair_escape(buf, ";index=", 7, hdr->index,
			     pc->pjsip_TOKEN_SPEC);
...
static pjsip_history_info_hdr* pjsip_history_info_hdr_clone( pj_pool_t *pool,
							     const pjsip_history_info_hdr *rhs)
{
    ...
    hdr->sname = rhs->sname;
    hdr->uri = (pjsip_uri*) pjsip_uri_clone(pool, rhs->uri);
    pj_strdup( pool, &hdr->index, &rhs->index);
    pjsip_param_clone( pool, &hdr->other_param, &rhs->other_param);

    return hdr;
}

static pjsip_history_info_hdr*
pjsip_history_info_hdr_shallow_clone( pj_pool_t *pool,
				const pjsip_history_info_hdr *rhs)
{
    pjsip_history_info_hdr *hdr = PJ_POOL_ALLOC_T(pool, pjsip_history_info_hdr);
    pj_memcpy(hdr, rhs, sizeof(*hdr));
    pjsip_param_shallow_clone( pool, &hdr->other_param, &rhs->other_param);
    return hdr;
}
